...
{
    // Convert libra:// to libra:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("libra://"))
    {
        uri.replace(0, 12, "libra:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "LIBRA.lnk";
}
...
{
    return GetAutostartDir() / "libra.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("LIBRA-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  libra-qt [" + tr("command-line options") + "]                     " + "\n";
